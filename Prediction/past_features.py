...
def features_past_generation(features_creation_function,
    ...
    for i, match_indice in enumerate(indices):
        ...
        past_matches = data[(data.Date < match.Date) & (
    ...
    train.columns = [feature_names_prefix +
                     str(i) for i in range(len(train.columns))]
    return train
...
def features_player_creation(outcome, match, past_matches):
    ...
    features_player += [len(wins_surface),
                        len(losses_surface), len(todo_surface)]
    per_victory_surface = 100 * \
        len(wins_surface) / len(todo_surface) if len(todo_surface) > 0 else np.nan
...
def features_recent_creation(outcome, match, past_matches):
    ...
    nswlmp = todo.iloc[-1,
                       :]['Wsets'] if wlmw == 1 else todo.iloc[-1, :]['Lsets']
...
def features_duo_creation(outcome, match, past):
    ...
    per_victory_player1 = 100 * \
        len(duo1) / len(duo) if len(duo) > 0 else np.nan
...
def features_general_creation(outcome, match, past_matches):
    ...
    best_ranking_as_winner = past_matches[(
        past_matches.Winner == player1)].WRank.min()
    best_ranking_as_loser = past_matches[(
        past_matches.Loser == player1)].LRank.min()
