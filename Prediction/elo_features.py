...
def compute_elo_rankings(data):
    ...
    players = list(pd.Series(list(data.Winner) +
                             list(data.Loser)).value_counts().index)
    ...
    ranking_elo = pd.DataFrame(ranking_elo, columns=[
                               "elo_winner", "elo_loser"])
    ranking_elo["proba_elo"] = 1 / \
        (1 + 10 ** ((ranking_elo["elo_loser"] -
                     ranking_elo["elo_winner"]) / 400))
    return ranking_elo
